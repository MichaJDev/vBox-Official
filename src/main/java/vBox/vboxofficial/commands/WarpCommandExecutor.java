...
import vBox.vboxofficial.data.yml.WarpHandler;
	...
	public boolean onCommand(CommandSender s, Command c, String l, String[] args) {
		...
		if (c.getName().equalsIgnoreCase("warp")) {
			if (!(s instanceof Player)) {
				LogHandler.writeToLog(s.getName() + " used /warp");
				if (s.hasPermission("vBox.warps")) {
					...
					if (c.getName().length() == 0) {
						...
						if (wh.Exists(args[0])) {
							...
							LogHandler.writeToLog(s.getName() + " used /warp " + args[0]);
							return false;
						} else {
			...
			if (!(s instanceof Player)) {
				...
				LogHandler.writeToLog(s.getName() + " used /warps");
				if (s.hasPermission("vBox.warps")) {
					if (args.length == 0) {
						...
						s.sendMessage(main.colorizeNoTag("&6+&c--&r| &6Warps &r|&c----------------&6+"));
						s.sendMessage(main.colorizeNoTag("&c" + sb.toString()));
						return false;
